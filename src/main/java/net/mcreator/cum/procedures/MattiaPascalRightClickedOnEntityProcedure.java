		...
		if (entity.isVehicle() && (entity instanceof MattiaPascalEntity _datEntL1 && _datEntL1.getEntityData().get(MattiaPascalEntity.DATA_story_sound)) == false) {
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("cum:ilfumattiastory")), SoundSource.NEUTRAL, 1, 1);
				} else {
					_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("cum:ilfumattiastory")), SoundSource.NEUTRAL, 1, 1, false);
				}
			}
