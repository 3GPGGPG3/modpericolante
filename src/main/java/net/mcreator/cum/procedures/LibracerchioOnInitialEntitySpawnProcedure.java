...
public class LibracerchioOnInitialEntitySpawnProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
			...
			return;
		{
			...
			if (!_ent.level().isClientSide() && _ent.getServer() != null) {
				_ent.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, _ent.position(), _ent.getRotationVector(), _ent.level() instanceof ServerLevel ? (ServerLevel) _ent.level() : null, 4,
						_ent.getName().getString(), _ent.getDisplayName(), _ent.level().getServer(), _ent), "tp @s @p");
			}
		}
		CumMod.queueServerWork(20, () -> {
			...
			if (entity instanceof LibracerchioEntity _datEntSetL)
				_datEntSetL.getEntityData().set(LibracerchioEntity.DATA_cerchio_in, true);
