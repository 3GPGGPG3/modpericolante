...
public class Lunare4dannoProcedure {
			...
			if (entity.isAlive()) {
				...
				CumMod.queueServerWork(15, () -> {
					if (entity.isAlive()) {
						...
						entity.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.MOB_ATTACK)), 9);
						if (Math.random() < 0.7) {
							CumMod.queueServerWork(15, () -> {
								if (entity.isAlive()) {
									{
												...
												if (Math.random() < 0.25) {
													CumMod.queueServerWork(15, () -> {
														if (entity.isAlive()) {
															...
															if (world instanceof Level _level) {
																if (!_level.isClientSide()) {
																	_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.evoker.prepare_summon")), SoundSource.AMBIENT, 2, 1);
																} else {
																	_level.playLocalSound(x, y, z, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.evoker.prepare_summon")), SoundSource.AMBIENT, 2, 1, false);
																}
															}
															entity.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.MOB_ATTACK)), 20);
														}
													});
												}
											}
